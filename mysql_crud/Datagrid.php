...
class Datagrid  extends Dataview{
	...
	*/
    ...
    function Datagrid($host="",$user="",$password="",$database="",$charset="") {
        parent::Dataview($host,$user,$password,$database,$charset);
    }
	/**
	 *生成查询的SQL语句
	 */
	...
	public function setSearchHtml() {
		...
		for($i=0;$i<$numTR;$i++){
			...
			for($j=0;$j<$numCos;$j++){
				...
				$this->searchHtml.='
			            <td  >'.(isset($searchValues[$num])?$searchValues[$num]:"").'</td>
			            <td >'.$opt.'</td>
			            <td  >'.$input.'</td>
						';
				$num++;
			...
			for($j=0;$j<$numCos;$j++){
				...
				if(isset($formFields[$num])){
					...
					if($flag!="view"){
						...
						$fieldHtml=$this->extendField($name,$fieldHtml,$value);
					}else{
	...
	public function getPageStr($pages,$page,$page_url_name="datagrid_page"){
		...
		if(!strstr($new_url,'?')){
			...
			if(!strstr($new_url,'var_name=')){
		                $new_url.='&var_name='.$var_name;
            		}
			//$new_url.='&var_name='.$var_name;
		}
        
        $new_url = str_replace("datagrid_action=edit","",$new_url);
        $new_url = str_replace("datagrid_action=new","",$new_url);        
