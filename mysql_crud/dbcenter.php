...
function get_db($type_name="default"){
    global $dbcache;
    if($dbcache[$type_name]){
        return $dbcache[$type_name];
    }
    ...
    $db_pass = $db[2];
    $db_name = $db[3];
    $dbcache[$type_name] = new MysqlDAO($db_host, $db_user, $db_pass, $db_name); 
    return $dbcache[$type_name];     
}
...
function print_table($data,$function=""){    
    ...
    foreach($data as $k){
        ...
        if($function){echo "<td>";call_user_func($function,$k);echo "</td>";};
        echo "</tr>\n";
    }
    echo "</table>";
}
