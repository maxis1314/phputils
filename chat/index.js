...
io.sockets.on('connection', function (socket) 
{
    ...
    var sendWelcome = function (info) {
        ...
        redis_obj.subscribe(socket.sub_r_id+'-'+info.user_name);
        console.log('subscribe to:'+socket.sub_r_id+'-'+info.user_name)
        //已经登陆
    ...
    {  
        //判断是否被禁言
        console.log(socket.forbidden_talk + socket.user_name)
    ...
    socket.on('kick_out', function(msg,fn)
    {
        ...
        msg_sender.publish(socket.sub_r_id + '-' + msg.to, JSON.stringify(msg));
        console.log(socket.sub_r_id + '-' + msg.to)
        fn(true);
    })
    ...
    socket.on('disconnect',function()
    {
        ...
        info.user_name = socket.user_name;
        info.nick_name = socket.nick_name;
        info.content = '退出';
