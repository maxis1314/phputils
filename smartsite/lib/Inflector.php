...
class Inflector {
	...
	function slug($string, $replacement = '_') {
		...
		$map = array (
			...
			'/[^\w\s]/' => ' ',
			'/\\s+/' => $replacement,
			String :: insert('/^[:replacement]+|[:replacement]+$/', array (
				...
				elseif (is_array($value)) {
					if ($primary === true) {
						...
						foreach ($value as $key2 => $value2) {
							$out-> {
								$key2 }
							= Set :: __map($value2, true);
						}
					} else {
						if (!is_numeric($key)) {
							$out-> {
								$key }
							= Set :: __map($value, true, $key);
							if (is_object($out-> {
								$key }) && !is_numeric($key)) {
								if (!isset ($out-> {
									$key }
