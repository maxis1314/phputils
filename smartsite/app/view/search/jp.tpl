...
<h2>Search Engine</h2>
<form method=GET>
<input type=text name=key id=key value="{$keyw|escape}" title="キーワードを入力して検索見よう"><input type=submit value="検索">
<br>
...
<tr> 
  ...
  <td align=right nowrap><font size=-1><b>{$keyw|escape}</b> の検索結果 <b>{$page*10}</b> - <b>{$page*10+10}</b> 件目  (<b>{$consumeTime}</b> 秒)&nbsp;</font></td> 
</tr> 
</table> 
...
{else}
 <b>{$keyw|escape}</b> に関するページを見つけられませんでした。
{/if}
{/if}
