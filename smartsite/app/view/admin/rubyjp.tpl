...
#navigation3{
	position:absolute;
	{/literal}{if $post.gongsi}bottom{else}bottom{/if}{literal}:0px;
	right:16px;
	width:98%;	
	height:{/literal}{if $post.gongsi}60{else}85{/if}{literal}px;
	text-align:left;
	background:#eee;
	z-index:2;
	overflow:hidden;
...
<div id="navigation2">
{if !$post.gongsi}
<iframe frameborder=0 height="300" margin-top=150 marginwidth=0 marginheight=0 hspace=0 vspace=0 scrolling=yes width="500" src="{$post.url|escape}"></iframe><br>
{/if}
</div>
...
<hr>
<form method=POST action="/admin/rubyjp/{$last_id|escape}/{$post.lang|escape}">
<a href="/admin/rubyjp/{$last_id|escape}/{$post.lang|escape}">SAVE</a>
<a href="/file">History</a>
<textarea name=ok>
{$post.ok|escape}
</textarea><a href="/error/rd?url={$post.url|escape}">[From..]</a>
...
<div id="navigation3">		
		<form method=POST action="/admin/rubyjp/{$last_id|escape}/{$post.lang|escape}">				
		<a href="/error/rd?url={$post.url|escape}">[From..]</a><a target=_blank href="/admin/jptest">[JPTEST]</a><input id="autosaveflag" type=checkbox value="aut"><input id="needtranslate" type=checkbox checked=true>
		{literal}
	...
	if(selected && ($("#autosaveflag").attr("checked") == false || document.getElementById("kannji2").value=="")){
		...
		if($("#needtranslate").attr("checked") == true){	
			...
			if("{/literal}{$post.lang|escape}{literal}"=="en"){			
				...
				if("{/literal}{$post.gongsi2}{literal}"){
...
function displayyahoo(text){	
	...
	if("{/literal}{$post.lang|escape}{literal}"!="en"){	
		var url="/admin/getjapanese2/"+text+"/{/literal}{$post.lang|escape}/{literal}";
...
function refresh(){
	if(needrefresh>0){
		...
		var url="/admin/rubyajax/{/literal}{$last_id|escape}/{$post.lang|escape}/?gongsi={$post.gongsi|escape}{literal}";
...
function loading(){
	if(needrefresh2){
		...
		if("{/literal}{$post.lang|escape}{literal}"=="fr"){
			google.language.translate(selected, "fr", "en", function(result) {  if (!result.error) {    google.language.translate(result.translation, "en", "zh-CN", function(result) {  if (!result.error) {    
				  ...
				  if(data!="" && data!=" " && $("#autosaveflag").attr("checked") == true && {/literal}"{$post.gongsi|escape}"{literal}=="1"){
				  	save();
		...
		if("{/literal}{$post.lang|escape}{literal}"=="enjp"){
		...
		var url="/admin/getjapanese/"+selected+"/{/literal}{$post.lang|escape}/{literal}";
		var url2="/admin/getjapanese2/"+selected+"/{/literal}{$post.lang|escape}/{literal}";
