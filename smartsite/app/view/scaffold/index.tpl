...
{/literal}
<h2>Scaffold　デモ&nbsp;<a href="{$v.prefix}/{$v.control}/code" target=_blank>Code</a></h2>


<form method=POST action='{$v.prefix}/{$v.control}/search'><input type=text name=key size=30 value="{$post.key|escape}"><input type=submit value="検索"><a href={$v.prefix}/{$v.control}/form>Advanced Search</a></form>
...
<table>
   <tr>
       ...
       <th><a href="{$v.prefix}/{$v.control}/?order={$col.name|urlencode}">{$col.name|escape}</a></th>
       {/if}
	   {/foreach}
	   <th>Operate</th>
   </tr>
     {foreach name=foo from=$list item=item}
      <tr id=row-{$smarty.foreach.foo.iteration}>
       ...
       {if $value!="created" && $value!="modified"}
       		{if $post.key}
	       		<td style="background-color:{$tr_color};">{$item.$value|substr2:0:6000|escape|nl2br|hilight:$post.key}
	       	{else}
	       		<td style="background-color:{$tr_color};">{$item.$value|substr2:0:600|escape|nl2br}
	       	{/if}
	       	{if $value=="url"}
	       		&nbsp;<a target=_blank href="/error/rd?url={$item.$value|urlencode}">-&gt;</a>
	       	{/if}
	       	</td>
	   {/if}
       {/foreach}
       <td style="background-color:{$tr_color};">
       [<a href="{$v.prefix}/{$v.control}/show/{$item.id}/">詳細</a>,<a href="{$v.prefix}/{$v.control}/showdetail/{$item.id}/">関連情報</a>,<a href="{$v.prefix}/{$v.control}/edit/{$item.id}/">編集</a>,<a href="{$v.prefix}/{$v.control}/add/{$item.id}/">コピー追加</a>,
